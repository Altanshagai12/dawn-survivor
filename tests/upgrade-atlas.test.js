...
const DECORATIVE_WIDTH = 76;
const DECORATIVE_HEIGHT = 84;
...
test('every upgrade uses one normalized decorative frame footprint', async () => {
  const file = fileURLToPath(new URL('../assets/ui/upgrade-icons.webp', import.meta.url));
  const { data, info } = await sharp(file).removeAlpha().raw().toBuffer({ resolveWithObject: true });
  for (let frame = 0; frame < 100; frame += 1) {
    const frameLeft = (frame % 10) * FRAME_SIZE;
    const frameTop = Math.floor(frame / 10) * FRAME_SIZE;
    let minX = FRAME_SIZE; let minY = FRAME_SIZE; let maxX = -1; let maxY = -1;
    for (let y = 0; y < FRAME_SIZE; y += 1) for (let x = 0; x < FRAME_SIZE; x += 1) {
      const offset = ((frameTop + y) * info.width + frameLeft + x) * info.channels;
      if (!isDecorativeFrame(data, offset)) continue;
      minX = Math.min(minX, x); minY = Math.min(minY, y); maxX = Math.max(maxX, x); maxY = Math.max(maxY, y);
    }
    assert.ok(Math.abs(maxX - minX + 1 - DECORATIVE_WIDTH) <= 1, `frame ${frame} must share one decorative width`);
    assert.ok(Math.abs(maxY - minY + 1 - DECORATIVE_HEIGHT) <= 1, `frame ${frame} must share one decorative height`);
    assert.ok(Math.abs((minX + maxX) / 2 - 47.5) <= 1, `frame ${frame} decorative border must be horizontally centered`);
    assert.ok(Math.abs((minY + maxY) / 2 - 47.5) <= 1, `frame ${frame} decorative border must be vertically centered`);
  }
});
