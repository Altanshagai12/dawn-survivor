...
import { PREMIUM_SKINS, normalizeSkinProfile } from '../../src/data/skins.js';
...
button.textContent = 'Run moving-skin replay suite';
...
const profile = normalizeSkinProfile(defaultProfile());
...
const game = new Phaser.Game({
  ...
  scene: [BootScene, GameScene], render: { antialias: true },
});
...
async function runMoving(selection, label, replay = false) {
  ...
  profile.equippedSkins[selection.heroId] = selection.skinId;
...
button.onclick = async () => {
  ...
  try {
    ...
    for (const skin of Object.values(PREMIUM_SKINS)) {
      const selection = { heroId: skin.heroId, weaponId: 'shotgun', skinId: skin.id };
    ...
    log('PASS: 25 runs. All eight premium skins and four original heroes move/fire after PLAY AGAIN; switching back to the first skin works.');
  } catch (error) {
